...
class MetricResult:
    ...
    # Row tracking
    total_rows: int
    affected_row_ids: List[int] = dataclass_field(default_factory=list)  # Row indices that violate
    
    # Metadata
    metadata: dict = dataclass_field(default_factory=dict)  # Extra context (null_count, distribution, etc)
...
class ValidationResult:
    ...
    # Details
    violating_row_ids: List[int] = dataclass_field(default_factory=list)  # Row indices that failed
    message: Optional[str] = None  # Human-readable explanation
    metadata: dict = dataclass_field(default_factory=dict)  # Extra context
    
    def __repr__(self):
        return f"ValidationResult({self.check_type} on {self.field}: {self.status.value})"    
