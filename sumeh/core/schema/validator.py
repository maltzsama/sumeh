...
TYPE_MAP = {
    ...
    'long': 'integer', 'bigint': 'integer', 'tinyint': 'integer', 'smallint': 'integer',
    ...
    'utf8': 'string', 'utf': 'string', 'categorical': 'string',
    
    # Boolean
    'bool': 'boolean', 'boolean': 'boolean',
    
    # DateTime
    'datetime': 'datetime', 'datetime64': 'datetime', 'timestamp': 'datetime',
    'date': 'datetime', 'time': 'datetime',
    
    # Complex types
    'list': 'array', 'array': 'array',
    'struct': 'complex', 'dict': 'complex', 'map': 'complex',
}
    ...
    if hasattr(obj, "schema") and hasattr(obj.schema, "fields"):
        for field in obj.schema.fields:
            field_data = {
                ...
                "comment": field.metadata.get("comment", "") if hasattr(field, "metadata") else "",
...
def extract_schema(obj: Any, as_json: bool = False) -> Union[Dict[str, Any], str]:
    ...
    if isinstance(obj, dict):
        extracted = {
            str(k): {"type": _to_canonical_type(str(v))}
            for k, v in obj.items()
        }
