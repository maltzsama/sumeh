...
def validate(df: dd.DataFrame, rules: List[RuleDef], baseline_provider=None) -> ValidationReport:
    ...
    try:
        if not lazy_metrics:
            return ValidationReport(results=[], total_rows=0, engine="dask", execution_time_ms=0)
        ...
        return ValidationReport(
            results=[], total_rows=0, engine="dask", 
    ...
    for i, rule in enumerate(active_rules):
        ...
        if rule.check_type.startswith("has_"):
            ...
            metric_value = (total_rows - fail_count) / total_rows if total_rows > 0 else 1.0
