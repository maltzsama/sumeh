...
class LegitAnalyzer:
    ...
    def analyze(df: pl.DataFrame, rule: RuleDef) -> MetricResult:
        ...
        total = len(df)
        fail_condition = pl.col(field).is_null() | (pl.col(field).cast(pl.Utf8).str.strip_chars() == "")
        fail_count = df.select(fail_condition.sum()).item()
        fail_row_ids = df.with_row_count().filter(fail_condition).select("row_nr").to_series().to_list()
        pass_rate = (total - fail_count) / total if total > 0 else 1.0
...
class DateAnalyzer:
    ...
    def analyze(df: pl.DataFrame, rule: RuleDef) -> MetricResult:
        ...
        if check_type == "is_today":
            ...
            fail_condition = pl.col(field).dt.date() != (today - datetime.timedelta(days=1))
        elif check_type == "is_t_minus_1":
            fail_condition = pl.col(field).dt.date() != (today - datetime.timedelta(days=1))
        elif check_type == "is_t_minus_2":
            fail_condition = pl.col(field).dt.date() != (today - datetime.timedelta(days=2))
        elif check_type == "is_t_minus_3":
            fail_condition = pl.col(field).dt.date() != (today - datetime.timedelta(days=3))
        elif check_type == "is_past_date":
            fail_condition = pl.col(field).dt.date() >= today
        elif check_type == "is_future_date":
            fail_condition = pl.col(field).dt.date() <= today
        elif check_type == "is_on_weekday":
            fail_condition = pl.col(field).dt.weekday().is_in([5, 6])  # Sat/Sun are weekend
        elif check_type == "is_on_weekend":
            fail_condition = pl.col(field).dt.weekday().is_in([0, 1, 2, 3, 4])  # Mon-Fri
        ...
        return MetricResult(
            ...
            total_rows=total,
            affected_row_ids=fail_row_ids,
            metadata={"fail_count": fail_count, "total_count": total, "check_type": check_type}
        )
...
class DateBetweenAnalyzer:
    ...
    def analyze(df: pl.DataFrame, rule: RuleDef) -> MetricResult:
        ...
        total = len(df)
        fail_condition = (pl.col(field).dt.date() < start) | (pl.col(field).dt.date() > end)
        fail_count = df.select(fail_condition.sum()).item()
        fail_row_ids = df.with_row_count().filter(fail_condition).select("row_nr").to_series().to_list()
        pass_rate = (total - fail_count) / total if total > 0 else 1.0
...
class AggregationAnalyzer:
    ...
    def analyze(df: pl.DataFrame, rule: RuleDef) -> MetricResult:
        ...
        return MetricResult(
            ...
            total_rows=len(df),
            affected_row_ids=[],
            metadata={"metric": check_type, "value": float(actual) if actual is not None else None}
        )