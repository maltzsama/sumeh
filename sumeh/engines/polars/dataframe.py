...
class ValidatedPolarsDataFrame:
    ...
    def split_by_errors(self, error_column: str = "_dq_errors") -> Tuple[pl.DataFrame, pl.DataFrame]:
        """
    ...
    def __repr__(self):
        ...
        errors = self._df.select(
            pl.col("_dq_errors").list.len().sum()
        ).item() if "_dq_errors" in self._df.columns else 0
