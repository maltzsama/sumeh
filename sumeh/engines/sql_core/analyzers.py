...
class MultiFieldCompletenessAnalyzer:
    ...
    def analyze(rule: RuleDef) -> exp.Expression:
        ...
        conditions = [
            exp.Not(this=exp.Is(this=_to_col(f), kind=exp.Null()))
            for f in fields
        ]
...
class UniquenessAnalyzer:
    ...
    def analyze(rule: RuleDef) -> exp.Expression:
        ...
        return exp.Cast(this=count_distinct, to=exp.DataType.build("double")) / count_all
...
class DateAnalyzer:
    ...
    def analyze(rule: RuleDef) -> exp.Expression:
        ...
        if check_type == "is_today":
            cond = exp.EQ(this=exp.DateTrunc(unit=exp.var("DAY"), this=col), expression=now)
            
        elif check_type in ["is_t_minus_1", "is_yesterday"]:
            yesterday = exp.DateSub(this=now, expression=exp.Literal.number(1))
            cond = exp.EQ(this=exp.DateTrunc(unit=exp.var("DAY"), this=col), expression=yesterday)
            
        elif check_type == "is_past_date":
            cond = exp.LT(this=exp.DateTrunc(unit=exp.var("DAY"), this=col), expression=now)
            
        elif check_type == "is_future_date":
            cond = exp.GT(this=exp.DateTrunc(unit=exp.var("DAY"), this=col), expression=now)
        ...
        # Placeholder for Weekday logic - relying on SQLGlot best effort
            ...
            cond = exp.In(this=dow, expressions=[exp.Literal.number(1), exp.Literal.number(7)])

        elif check_type == "is_on_weekday":
             dow = exp.DayOfWeek(this=col)
             cond = exp.Not(this=exp.In(this=dow, expressions=[exp.Literal.number(1), exp.Literal.number(7)]))
...
class DateBetweenAnalyzer:
    ...
    def analyze(rule: RuleDef) -> exp.Expression:
        ...
        start = exp.Cast(this=exp.Literal.string(rule.value[0]), to=exp.DataType.build("date"))
        end = exp.Cast(this=exp.Literal.string(rule.value[1]), to=exp.DataType.build("date"))
...
class DateComparisonAnalyzer:
    ...
    def analyze(rule: RuleDef) -> exp.Expression:
        col = _to_col(rule.field)
        target = exp.Cast(this=exp.Literal.string(rule.value), to=exp.DataType.build("date"))
