"""
Tests for sumeh/engines/pandas/
...
class TestComparisonAnalyzer:
    ...
    def test_is_greater_than(self):
        df = pd.DataFrame({"score": [80, 90, 40, 70]})
        rule = RuleDefinition(field="score", check_type="is_greater_than", value=50)
        metric = ComparisonAnalyzer.analyze(df, rule)
        assert metric.value == pytest.approx(0.75)

    def test_is_less_than(self):
        df = pd.DataFrame({"age": [20, 30, 150, 45]})
        rule = RuleDefinition(field="age", check_type="is_less_than", value=100)
        metric = ComparisonAnalyzer.analyze(df, rule)
        assert metric.value == pytest.approx(0.75)

    def test_is_greater_or_equal_than(self):
        df = pd.DataFrame({"score": [70, 80, 60, 90]})
        rule = RuleDefinition(
            field="score", check_type="is_greater_or_equal_than", value=70
        )
        metric = ComparisonAnalyzer.analyze(df, rule)
        assert metric.value == pytest.approx(0.75)

    def test_is_less_or_equal_than(self):
        df = pd.DataFrame({"age": [18, 25, 121, 30]})
        rule = RuleDefinition(
            field="age", check_type="is_less_or_equal_than", value=120
        )
        metric = ComparisonAnalyzer.analyze(df, rule)
        assert metric.value == pytest.approx(0.75)

    def test_is_equal(self):
        df = pd.DataFrame({"score": [100, 100, 90, 100]})
        rule = RuleDefinition(field="score", check_type="is_equal", value=100)
        metric = ComparisonAnalyzer.analyze(df, rule)
        assert metric.value == pytest.approx(0.75)

    def test_is_in_millions(self):
        df = pd.DataFrame({"val": [1_000_000, 2_000_000, 500, 1_500_000]})
        rule = RuleDefinition(field="val", check_type="is_in_millions")
        metric = ComparisonAnalyzer.analyze(df, rule)
        assert metric.value == pytest.approx(0.75)

    def test_is_in_billions(self):
        df = pd.DataFrame({"val": [1_000_000_000, 500_000, 2_000_000_000]})
        rule = RuleDefinition(field="val", check_type="is_in_billions")
        metric = ComparisonAnalyzer.analyze(df, rule)
        assert metric.value == pytest.approx(2 / 3)
...
class TestBetweenAnalyzer:
    ...
    def test_some_out_of_range(self):
        df = pd.DataFrame({"age": [20, 30, 5, 200]})
        rule = RuleDefinition(field="age", check_type="is_between", value=[18, 120])
        metric = BetweenAnalyzer.analyze(df, rule)
...
class TestMembershipAnalyzer:
    ...
    def test_is_contained_in_fail(self):
        df = pd.DataFrame({"status": ["active", "banned", "pending"]})
        rule = RuleDefinition(
            field="status",
            check_type="is_contained_in",
            value=["active", "inactive", "pending"],
    ...
    def test_not_contained_in_fail(self):
        df = pd.DataFrame({"status": ["active", "banned", "deleted"]})
        rule = RuleDefinition(
            field="status", check_type="not_contained_in", value=["banned", "deleted"]
        )
        metric = MembershipAnalyzer.analyze(df, rule)
...
class TestPatternAnalyzer:
    ...
    def test_all_match(self):
        df = pd.DataFrame({"email": ["a@a.com", "b@b.com", "c@c.com"]})
        rule = RuleDefinition(
            field="email",
    ...
    def test_some_fail(self):
        df = pd.DataFrame({"email": ["a@a.com", "not-an-email", "c@c.com"]})
        rule = RuleDefinition(
            field="email",
            check_type="has_pattern",
            value=r"^[\w.-]+@[\w.-]+\.[a-zA-Z]{2,}$",
...
class TestDateAnalyzer:
    ...
    def test_is_today_pass(self):
        today = pd.Timestamp.now().normalize()
        df = pd.DataFrame({"dt": [today, today, today]})
    ...
    def test_is_today_fail(self):
        ...
        df = pd.DataFrame({"dt": [yesterday, yesterday]})
        rule = RuleDefinition(field="dt", check_type="is_today")
        metric = DateAnalyzer.analyze(df, rule)
    ...
    def test_is_past_date_pass(self):
        df = pd.DataFrame(
            {"dt": [pd.Timestamp("2020-01-01"), pd.Timestamp("2019-06-15")]}
        )
    ...
    def test_is_future_date_pass(self):
        ...
        df = pd.DataFrame({"dt": [future, future]})
    ...
    def test_is_on_weekday(self):
        # Monday = weekday 0
        monday = pd.Timestamp("2024-01-01")  # known Monday
        saturday = pd.Timestamp("2024-01-06")  # known Saturday
        df = pd.DataFrame({"dt": [monday, saturday]})
...
class TestDateBetweenAnalyzer:
    ...
    def test_all_within_range(self):
        df = pd.DataFrame(
            {"dt": [pd.Timestamp("2022-01-01"), pd.Timestamp("2023-06-15")]}
        )
        rule = RuleDefinition(
    ...
    def test_some_outside_range(self):
        df = pd.DataFrame(
            {"dt": [pd.Timestamp("2018-01-01"), pd.Timestamp("2023-06-15")]}
        )
        rule = RuleDefinition(
            field="dt", check_type="is_date_between", value=["2020-01-01", "2024-12-31"]
        )
    ...
    def test_invalid_bounds_raises(self):
        df = pd.DataFrame({"dt": [pd.Timestamp("2022-01-01")]})
        rule = RuleDefinition(
...
class TestDateComparisonAnalyzer:
    ...
    def test_is_date_after_fail(self):
        df = pd.DataFrame(
            {"dt": [pd.Timestamp("2018-01-01"), pd.Timestamp("2023-06-15")]}
        )
        rule = RuleDefinition(
...
class TestPandasValidate:
    ...
    def test_engine_is_pandas(self, df_base):
        rules = [RuleDefinition(field="user_id", check_type="is_unique")]
        report = validate(df_base, rules)
    ...
    def test_rule_fails_correctly(self):
        df = pd.DataFrame({"email": ["a@a.com", None, "c@c.com"]})
    ...
    def test_bad_df_has_dq_errors_column(self):
        df = pd.DataFrame(
            {
                "user_id": [1, 2, 3],
                "email": ["a@a.com", None, "c@c.com"],
        ...
        report = validate(df, rules)
        _, bad = report.split()
        assert "_dq_errors" in bad.columns

    def test_table_level_rules_in_report(self):
        df = pd.DataFrame({"age": [20, 30, 40, 50, 60]})
        rules = [
            RuleDefinition(
    ...
    def test_execute_false_skips_rule(self, df_base):
        rules = [RuleDefinition(field="email", check_type="is_complete", execute=False)]
        report = validate(df_base, rules)
