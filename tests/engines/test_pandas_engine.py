...
class TestPandasEngine(unittest.TestCase):
    ...
    def test_is_date_between(self):
        rule = self.create_rule(
            "date_col", "is_date_between", value="[2023-01-01, 2023-12-31]"
        )
    ...
    def test_with_threshold(self):
        rule = self.create_rule(
            "numeric_col", "has_mean", value=10.0, threshold=0.8, level="TABLE"
        )
    ...
    def test_is_complete_with_nulls_and_empty_strings(self):
        ...
        df = pd.DataFrame({"col1": [1, None, 3, None], "col2": ["A", "", None, "D"]})
    ...
    def test_is_legit_with_various_invalid_values(self):
        ...
        df = pd.DataFrame({"col": [None, " ", "", "valid", "  ", "\t", "valid2"]})
...
def test_is_future_date_pytest():
    ...
    df = pd.DataFrame(
        {
            "date_col": pd.to_datetime(
                ["2030-01-01", "2020-01-01", None]  # Futuro  # Passado
            )
        }
    )
