...
@pytest.mark.parametrize("column,sql", [
    ("rev", "rev >= 1000000"),
    ("x",   "x >= 1000000"),
])
...
@pytest.mark.parametrize("fn,name,expected", [
    ...
    (_is_on_weekend,   "is_on_weekend",   "(EXTRACT(DOW FROM col) = 0 OR EXTRACT(DOW FROM col) = 6)"),
