...
def sample_rules():
    return [
        {"field": "age", "check_type": "is_positive", "value": "", "threshold": 1.0, "execute": True},
        {"field": "email", "check_type": "has_pattern", "value": r"^[\w\.-]+@[\w\.-]+\.\w+$", "threshold": 0.9},
        {"field": "department", "check_type": "is_contained_in", "value": "[HR,IT,Finance,Sales]", "threshold": 1.0},
        {"field": "salary", "check_type": "is_between", "value": "[3000,10000]", "threshold": 1.0},
        {"field": ["name", "email"], "check_type": "are_unique", "value": "", "threshold": 1.0}
    ]
...
def test_has_pattern(sample_data):
    rule = {"field": "email", "check_type": "has_pattern", "value": r"^[\w\.-]+@[\w\.-]+\.\w+$"}
...
def test_summarize(sample_data, sample_rules):
    ...
    for row in summary.iter_rows(named=True):
        ...
        assert abs(row["pass_rate"] - expected_pass_rate) < 0.0001  # Tolerância para float
