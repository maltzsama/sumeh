...
class TestQualityFunction(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.spark = (
            SparkSession.builder.master("local[2]").appName("testes").getOrCreate()
        )

        cls.mock_data_file = os.path.join(os.path.dirname(__file__), "mock", "data.csv")
        cls.mock_config_file = get_config_from_csv(
            file_path=os.path.join(os.path.dirname(__file__), "mock", "config.csv"),
            delimiter=";",
        )
